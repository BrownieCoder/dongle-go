...
> **当前是开发预览，尚未通过真机验收。** 目前没有任何硬件与系统组合完成真机兼容性验证。请不要仅凭本项目购买模块。支持的配置将在完成[真机测试](docs/acceptance.md)后列出；软件模拟通过不等于硬件可用。
...
新版采用漫画猫、圆角设备卡片及中英界面，详见[界面更新](docs/ui-redesign.md)。[验证记录](docs/verification.md)包含 Windows/macOS 打包启动及 Linux 测试证据；真机验收独立进行，目前尚未通过。
